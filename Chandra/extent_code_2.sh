...
parameters=$(echo "${parameters_nom}" | awk '{print $3}' )
read -ra nom_split <<< ${parameters}

for filename in `ls -a ${pfolder}/${obsid}/repro`
do
        if [ "${filename##*.}"x = "dat_dithered"x ];then
                file=${pfolder}/${obsid}/repro/${filename}
		filename_special=${filename}
        fi
done
## get the celestial coordinates of the source for the PSF simulation, ra, dec, double, deg
...
parameters=$(echo "${parameters_src}" | awk '{print $3}' )
read -ra src_split <<< ${parameters}
...
punlearn dmcoords
dmcoords acisf0${obsid1}_repro_evt2.fits op=cel ra=${src_split[0]} dec=${src_split[1]} celfmt=deg
pget dmcoords x y
xy=$(pget dmcoords x y)
read -ra xy_split <<< ${xy}
deltax1=$(echo "scale=1; (${xy_split[0]}-256)/1.0" | bc)
deltax2=$(echo "scale=1; (${xy_split[0]}+256)/1.0" | bc)
deltay1=$(echo "scale=1; (${xy_split[1]}-256)/1.0" | bc)
deltay2=$(echo "scale=1; (${xy_split[1]}+256)/1.0" | bc)
## 将psf源放置在中心，将psf图像的大小设置为512*512
dmcopy "marx_output_i0000.fits[bin x=${deltax1}:${deltax2}:1,y=${deltay1}:${deltay2}:1]" psf_512_i0000.fits
# read -p 'enter the ccd_id ' ccd_id
## python查看fits文件可知用到哪块ccd，创建了真实数据的图像文件，
## 去掉了ccd_id=6，不限制（只要程序能跑就没问题）
apply_fov_limits "acisf0${obsid1}_repro_evt2.fits[energy=400:6000]" bin1.fits bin=1
...
do 
	marx @@./marx OutputDir=marx_output_${d}.dir
	SAOSACFile=${filename_special}_${d}_rays.fits
	marx2fits --pixadj=EDSER marx_output_${d}.dir marx_output_${d}.fits
done
dmmerge "marx_output_i00*.fits" marx_output.fits
echo "counts in psf_match_i0000.fits"
dmlist marx_output.fits counts
dmcopy "marx_output.fits[bin ${pixels}]" cts_psf.fits clob+ 
